...
export class Migrations1769853880290 implements MigrationInterface {
  name = 'Migrations1769853880290';

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "users" ADD "checkedAirdropChapter2" boolean NOT NULL DEFAULT false`,
    );
  ...
  `);
    ...
    await queryRunner.query(
      `SELECT setval('airdrop_chests_chapter2_id_seq', 5, true)`,
    );
...
`);
    ...
    await queryRunner.query(
      `SELECT setval('airdrop_tiers_chapter2_id_seq', 23, true)`,
    );
...
`);
    ...
    await queryRunner.query(
      `CREATE INDEX "IDX_airdrop_users_chapter2_address" ON "airdrop_users_chapter2" ("address")`,
    );
...
`);
    ...
    await queryRunner.query(
      `CREATE INDEX "IDX_airdrop_users_chests_chapter2_address" ON "airdrop_users_chests_chapter2" ("address")`,
    );
    await queryRunner.query(
      `CREATE INDEX "IDX_airdrop_users_chests_chapter2_status" ON "airdrop_users_chests_chapter2" ("status")`,
    );
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`DROP TABLE "airdrop_users_chests_chapter2"`);
    await queryRunner.query(`DROP TYPE "users_chests_status_chapter2"`);
    await queryRunner.query(`DROP TABLE "airdrop_users_chapter2"`);
    await queryRunner.query(
      `DROP TABLE "airdrop_tiers_delivery_rules_chapter2"`,
    );
    await queryRunner.query(`DROP TABLE "airdrop_tiers_chapter2"`);
    await queryRunner.query(`DROP TYPE "airdrop_tiers_name_chapter2"`);
    await queryRunner.query(`DROP TABLE "airdrop_chests_chapter2"`);
    await queryRunner.query(`DROP TYPE "airdrop_chests_type_chapter2"`);

    await queryRunner.query(
      `ALTER TABLE "users" DROP COLUMN "checkedAirdropChapter2"`,
    );
  }
}
