    ...
    (0, typeorm_1.ViewEntity)({
        expression: (dataSource) => {
            return (dataSource
                ...
                // .orderBy('"order"."collectionAddress"')
                // .addOrderBy('"order"."tokenIds"')
                .addOrderBy('"order"."price"', 'DESC'), 'sellNow', '"sellNow"."collectionAddress" = "item"."collectionAddress" AND ("item"."tokenId"::TEXT = ANY("sellNow"."tokenIds") OR "sellNow"."tokenIds" IS NULL)')
                .leftJoin((q) => q
                .from(__1.ActiveOrderCachedEntity, 'order')
                .select('"order".*')
                .addSelect((query) => query
